...
{	
	...
	void OnGUI() {
		...
		if(!isRunning)
		{
			...
			if(isFinished)
			{
				message = "Click or Press W to Play Again";
			}
			else
			{
				message = "Click or Press W to Play";
			}

			//Define a new rectangle for the UI on the screen
			Rect startButton = new Rect(Screen.width/2 - 120, Screen.height/2, 240, 30);

			if (GUI.Button(startButton, message) || Input.GetKeyDown(KeyCode.W))
			{
		...
		if(isFinished)
		{
			...
			GUI.Label(new Rect(Screen.width / 2 - 10, 200, 45, 30), ((double)elapsedTime).ToString("F3"));
		}
		else if(isRunning)
		{ 
			// If the game is running, show the current time
			GUI.Box(new Rect(Screen.width / 2 - 65, Screen.height - 115, 130, 40), "Your Time Is");
			GUI.Label(new Rect(Screen.width / 2 - 10, Screen.height - 100, 45, 30), ((double)elapsedTime).ToString("F3"));
		}
	}
}
